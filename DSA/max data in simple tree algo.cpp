...
//returns the maximum data node in tree
int maxdataintree(treenode<int>* root)
{
    ...
    {
        maxer=max(maxer, maxdataintree(x));
    }
    return maxer;
}

//returns sum of nodes in tree
int sumofnodesintree(treenode<int>* root)
{
    int temp=root->data;
    for(auto x:root->children)
    {
        temp+=sumofnodesintree(x);
    }
    return temp;
}
...
int main()
{
    ...
    cout<<"sum of nodes in tree is "<<sumofnodesintree(root)<<endl;
    cout<<"max data in tree "<<maxdataintree(root)<<endl;
